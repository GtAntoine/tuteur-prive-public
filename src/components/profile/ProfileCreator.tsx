import React, { useState } from 'react';
import { ArrowLeft } from 'lucide-react';
import { AvatarSelector } from './AvatarSelector';
import type { Grade } from '../../lib/types';

const GRADES: Grade[] = ['CP', 'CE1', 'CE2', 'CM1', 'CM2', '6ème', '5ème', '4ème', '3ème'];
const AVATAR_COLORS = ['red', 'orange', 'yellow', 'green', 'blue', 'indigo', 'purple', 'pink'];

interface ProfileCreatorProps {
  onSubmit: (name: string, grade: Grade, avatar_color: string, avatar_id?: string) => void;
  onBack: () => void;
}

export function ProfileCreator({ onSubmit, onBack }: ProfileCreatorProps) {
  const [name, setName] = useState('');
  const [grade, setGrade] = useState<Grade>('CP');
  const [avatar_color, setAvatarColor] = useState(AVATAR_COLORS[0]);
  const [avatar_id, setAvatarId] = useState<string>();

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim() && grade && avatar_color) {
      onSubmit(name.trim(), grade, avatar_color, avatar_id);
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 to-purple-900 flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full">
        <button
          onClick={onBack}
          className="flex items-center gap-2 text-white mb-8 hover:text-gray-200"
        >
          <ArrowLeft className="w-5 h-5" />
          Retour
        </button>

        <h1 className="text-3xl font-bold text-white mb-8">Créer un profil</h1>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="name" className="block text-white mb-2">
              Prénom
            </label>
            <input
              type="text"
              id="name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="w-full px-4 py-2 rounded-lg bg-white/10 text-white border border-white/20 focus:border-white/40 focus:outline-none"
              required
            />
          </div>

       
          <div>
            <label className="block text-white mb-2">
              Classe
            </label>
            <div className="grid grid-cols-3 gap-2">
              {GRADES.map((g) => (
                <button
                  key={g}
                  type="button"
                  onClick={() => setGrade(g)}
                  className={`px-4 py-2 rounded-lg transition-colors ${
                    grade === g
                      ? 'bg-white text-purple-900'
                      : 'bg-white/10 text-white hover:bg-white/20'
                  }`}
                >
                  {g}
                </button>
              ))}
            </div>
          </div>

           <div className="bg-white/10 p-4 rounded-lg">
            <AvatarSelector
              selectedAvatar={avatar_id}
              onSelectAvatar={setAvatarId}
            />
          </div>


          <div>
            <label className="block text-white mb-2">
              Couleur de fond de l'avatar
            </label>
            <div className="flex justify-between md:flex-wrap md:gap-2">
              {AVATAR_COLORS.map((color) => (
                <button
                  key={color}
                  type="button"
                  onClick={() => setAvatarColor(color)}
                  className={`w-10 h-10 rounded-full bg-${color}-300 transition-transform ${
                    avatarColor === color ? 'scale-110 ring-2 ring-white' : 'hover:scale-105'
                  }`}
                />
              ))}
            </div>
          </div>

          <div className="py-6">
            <button
              type="submit"
              disabled={!name.trim()}
              className="w-full px-6 py-3 bg-white text-purple-900 rounded-lg font-medium 
                transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Créer le profil
            </button>
            </div>
        </form>
      </div>
    </div>
      
  );
}